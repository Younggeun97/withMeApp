...
class HomeFragment : Fragment(), OnMapReadyCallback, GoogleMap.OnPolylineClickListener,GoogleMap.OnPolygonClickListener {
...
//        return inflater.inflate(R.layout.fragment_home, container, false)
        ...
        binding.stopbtn.setOnClickListener {
            ...
            if (isstopbtnClick == true) { // 시작 버튼이 눌렸는데 유저가 다시 한번 누른 경우
                Toast.makeText(activity, "이미 정지되었습니다.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
        ...
        binding.resetbtn.setOnClickListener {
            ...
            if (isstartbtnClick == true) { // 시작 버튼이 눌렸는데 유저가 다시 한번 누른 경우
                Toast.makeText(activity, "정지버튼을 누른 후 초기화해주세요.", Toast.LENGTH_SHORT).show()
            }
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        // googleMap객체 생성 -> googleMap을 이용하여 마커나 카메라,선 등을 조정
        mMap = googleMap
        // 서울을 기준으로 위도,경도 지정
        val marker = LatLng(37.56, 126.97)
        // MarkerOptions에 위치 정보를 넣기
